...
function panel(m) {
  ...
  //ajax传递数据到后台并获得批注id
  app.request.post('http://10.162.34.26:8080/addAnnotation', {
    content: textarea,
    paragraph: paragraph,
    start: start,
    end: end,
...
function getPassage(dele) { //dele用来标记是否需要重新渲染侧边栏
  $.ajax({
    url: 'http://10.162.34.26:8080/getPassage?courseId=3',
    type: "post",
    cache: true,
...
//   app.request.get('http://10.0.0.53:8080/getAnnotations?courseId=1&userId=1',
//     function (data) {
//       console.log(data);
...
function getAnnotator(passageId, userId, dele) {
  $.ajax({
    url: 'http://10.162.34.26:8080/getAnnotations?courseId=1&userId=1',
    type: "get",
    cache: true,
    success: function (data) {
      each(data, dele);
